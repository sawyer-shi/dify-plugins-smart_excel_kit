...
class SingleColumnTextAnalysisTool(Tool):
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        ...
        for i in target_rows:
            ...
            # === 核心修改: 优先写入 Workbook 以保留格式 ===
            if ws:
                try:
                    # excel row = pandas index(i) + 2 (1 for 0-index conversion, 1 for header)
                    ws.cell(row=i+2, column=out_info['col_idx']+1).value = result
                except:
                    # fallback
